...
function populateStats() {
  ...
  document.querySelector("#emergencyCount").textContent = String(data.phrases.filter((phrase) => phrase.isEmergency).length);
}
...
function buildPhraseCard(phrase) {
  ...
  article.innerHTML = `
    <div class="phrase-top">
      <div class="chip ${phrase.isEmergency ? "emergency" : ""}">${phrase.isEmergency ? "긴급" : category.title}</div>
...
function createActionButton(label, onClick, primary = false, background = "#1f2328") {
  const button = document.createElement("button");
